	...
	{{#each jobs}}
		<div class="candidate-section" id="{{jobType.guid}}">
			{{#hasUnarchivedCandidates this}}
				<div class="grid-list-head expanded">{{jobName}} ({{totalUnarchivedCandidatesByJob this}})</div>
				...
				{{#each candidates}}
					{{#if_eq archived false}}
						<li class="view-profile" data-id="{{id}}" data-guid="{{guid}}" data-user="{{user.guid}}" data-email="{{user.email}}">
							...
							<div class="candidate-endorse" title="Total number of Endorsements">{{user.endorsementCount}}</div>
							<div class="candidate-network sync" title="Total number of Connections you share">{{#if_gt user.sharedNetworkConnectionCount 500}}500+{{else}}{{user.sharedNetworkConnectionCount}}{{/if_gt}}</div>
							<div class="candidate-referral" title="Candidate's total number of Referrals">{{referralCount}}</div>
...
{{#anyArchivedCandidates jobs sub}}
	<div id="archived-candidates-list-container">
		<div class="grid-list-head expanded">{{language.archived}} ({{totalArchivedCandidates jobs sub}})</div>
